...
## Google Drive 연동 (개발 완료 후)

지금은 `STORAGE_DRIVER=local` 이라 서버 디스크에 저장한다. 실제 Drive 로 바꾸려면:

1. Google Cloud Console 에서 OAuth 클라이언트(웹 애플리케이션)를 만든다.
2. `https://www.googleapis.com/auth/drive` 스코프로 **리프레시 토큰**을 한 번 받아 둔다.
